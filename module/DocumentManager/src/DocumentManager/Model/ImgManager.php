...
/**
 * ImgManager provides helper functions to manage server-side memory and file legibility.
 *
...
class ImgManager {
    ...
    /**
     * Get max upload value from php config
     * 
     * @return string
     */
    public static function getMaxUploadMessage(){
        return ini_get('upload_max_filesize');
    }

    /**
     * Get allowed maximum upload size
     * 
     * @return 
     */
    public static function getMaxUploadSize() {
        return self::getByte(ini_get('upload_max_filesize'));
    }

    /**
     * Convert file size in string as size in byte
     * 
     * @param string $size
     * @return integer 
     */
    ...
    /**
     * Convert integer to short string as byte-size
     * 
     * @param int $size
     ...
     * @return string 
     */
    public static function getByteStr($size = 0, $threshold = 0.9, $decimal = 2) {
		
		//set default threshold to a Windows style of FileSizes
		$threshold = min(max($threshold, 0.75), 1); //should be between 75-100%

		$decimal = min(max($decimal, 0), 4); //should be between 0-4

		if($size < 1024 * $threshold){ //pow(2,10)
			return $size;// + " bytes";

		}else if($size < 1048576 * $threshold) { //pow(2,20)
			return (round((($size * pow(10, $decimal)) / 1024)) / pow(10, $decimal)) + "K";

		}else if(size < 1073741824*threshold) { //pow(2,30)
			return (round((($size * pow(10, $decimal)) / 1048576)) / pow(10, $decimal)) + "M";

		}else if(size < 1099511627776*threshold) { //pow(2,40)
			return (round((($size * pow(10, $decimal)) / 1073741824)) / pow(10, $decimal)) + "G";

		}else if(size < 1125899906842624*threshold) { //pow(2,50)
			return (round((($size * pow(10, $decimal)) / 1099511627776)) / pow(10, $decimal)) + "T";

		}else{ //pow(2,60)
			return (round((($size * pow(10, $decimal)) / 1125899906842624)) / pow(10, $decimal)) + "P";

		}
    }

	//********************************************************************************
	public static function getMemoryForImage($filename){
		$memoryNeeded = 0;
		if( isset($filename) ) {
			$imageInfo = getimagesize($filename);
			$fileEXT = \mb_substr(strrchr($filename, "."), 1); //much faster & safer than pathinfo!
			switch (strtolower($fileEXT)) {
				...
				case "jpeg":
					...
					break;
				case "gif":
					$memoryNeeded = round(($imageInfo[0] * $imageInfo[1] * $imageInfo['bits'] + Pow(2, 16)) * 1.65);
				case "png":
				//var_dump($imageInfo);
					$memoryNeeded = round(($imageInfo[0] * $imageInfo[1] * $imageInfo['bits'] + Pow(2, 16)) * 1.8);
	...
	public static function setMemoryForImage($filename){
		if( isset($filename) ) {
			$imageInfo    = getimagesize($filename);
			$memoryNeeded = self::getMemoryForImage($filename);
			
			$memoryLimit = self::getByte(ini_get('memory_limit'));
	...
	public function image_mirror(&$input_image_resource, $direction="vertical"){
	...
	public function imgInsertImage(&$srcImg,&$insertImg,$src_x,$src_y){
	...
	public function imgInsertText(&$srcImg,$textInsert,$textX,$textY,$textColor="000000", $textFont=3){
	...
	public function imgFileSize($filename){
	...
	public function imageInsertWatermark($destImage, $waterImage, $destX=0, $destY=0, $waterOpacity=50, $autoFitWater=true){
	...
	*/
	...
	public function getGIFColorMap($file){
	   $fp = fopen($file, 'r');
	   $buf = fread($fp, 1024);
	   fclose($fp);
	...
	public function ImageStringWrap($image, $font, $x, $y, $text, $color, $maxwidth){
	 ...
	 */
	public function fetch_gif_data($file) {
	...
	*/
	...
	public function setJPEGresolution($filename){
	/*
